...
class SearchOperation : public QObject
{
  ...
  protected:
    ...
    std::shared_ptr<const FilterSet> filterSet_;
...
{
  public:
    FullSearchOperation( const LogData* sourceLogData, std::shared_ptr<const FilterSet> filterSet,
            bool* interruptRequest )
        : SearchOperation( sourceLogData, filterSet, interruptRequest ) {}
...
{
  public:
    UpdateSearchOperation( const LogData* sourceLogData, std::shared_ptr<const FilterSet> filterSet,
...
class LogFilteredDataWorkerThread : public QThread
{
  ...
  public:
    ...
    void search( std::shared_ptr<const FilterSet> filterSet );
    // Continue the previous search starting at the passed position
    // in the source file (line number)
    void updateSearch( std::shared_ptr<const FilterSet> filterSet, qint64 position );
